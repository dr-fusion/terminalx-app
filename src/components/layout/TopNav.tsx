...
export function TopNav({ onOpenPalette }: TopNavProps) {
  ...
  return (
    <div
      className="h-12 flex items-center gap-5 px-4 border-b border-[#1a1d24] bg-[#0a0b10] shrink-0"
      style={{ position: "sticky", top: 0, zIndex: 50 }}
    >
      <Link
        href="/dashboard"
        className="flex items-baseline gap-0 text-[16px] font-bold tracking-tight text-[#e6f0e4]"
      >
        ...
        {tabs.map((t) => {
          ...
          return (
            <Link
              key={t.href}
              href={t.href}
              className={`px-3 flex items-center text-[11px] relative ${
      ...
      <button
        ...
        className="flex items-center gap-2 bg-[#14161e] border border-[#252933] hover:border-[#363b47]
          rounded px-2.5 py-1.5 text-[10px] text-[#6b7569] transition-colors"
      ...
      {user && (
        <div className="flex items-center gap-2 pl-3 border-l border-[#1a1d24]">
          ...
          <button
            onClick={logout}
            className="text-[10px] text-[#6b7569] hover:text-[#ff5c5c] transition-colors"
          >
            sign out
          </button>
        </div>
