...
/*
 * The Secret Broker's accepted Unix socket is inherited only as fd 3. stdout
 * carries one fixed-shape record; peer-controlled bytes never enter it. Unlike
 * the sandbox peercred helper this runs as the broker's own (non-root) service
 * uid, so it does not require euid 0.
 */
int main(int argc, char **argv) {
  ...
  (void)argv;
  if (argc != 1 || fcntl(3, F_GETFD) < 0 || fstat(3, &socket_stat) != 0 ||
